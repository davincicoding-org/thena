...
CREATE TYPE "public"."task_run_status" AS ENUM('pending', 'skipped', 'completed');--> statement-breakpoint
...
CREATE TABLE "task_runs" (
	...
	"timestamps" timestamp with time zone[] DEFAULT '{}',
	"duration" integer
);
--> statement-breakpoint
CREATE TABLE "tasks" (
	...
	"sort_order" double precision GENERATED ALWAYS AS (COALESCE("tasks"."order", "tasks"."id")) STORED NOT NULL,
	"parent_id" integer,
	"project_id" integer,
	...
	CONSTRAINT "title_min_length" CHECK (LENGTH(TRIM("tasks"."title")) >= 3)
);
--> statement-breakpoint
ALTER TABLE "task_runs" ADD CONSTRAINT "task_runs_sprint_id_sprints_id_fk" FOREIGN KEY ("sprint_id") REFERENCES "public"."sprints"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "idx_sprints_owner" ON "sprints" USING btree ("user_id");--> statement-breakpoint
CREATE INDEX "idx_sprints_ended_at" ON "sprints" USING btree ("ended_at");--> statement-breakpoint
CREATE INDEX "idx_task_run_sprint_id" ON "task_runs" USING btree ("sprint_id");--> statement-breakpoint

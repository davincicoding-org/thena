...
export function useProjects(): ProjectsHookReturn {
  ...
  const projects = Object.entries(projectsStore.pool).map(([id, project]) => ({
    id,
    ...project,
  }));

  const createProject: ProjectsHookReturn["createProject"] = (
    { imageFile, ...input },
    callback,
  ) => {
  ...
  const updateProject: ProjectsHookReturn["updateProject"] = (
    projectId,
    updates,
  ) => {
  ...
  const deleteProject: ProjectsHookReturn["deleteProject"] = (projectId) => {
